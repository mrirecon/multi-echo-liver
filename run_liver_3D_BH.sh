...
# for stack-of-stars multi-echo breath-hold liver acquisition
# 
...
R_FILE="R_WFR2S_3D_BH"

NBIN=$(bart show -d 10 kdat_prep)
NPAR=$(bart show -d 13 kdat_prep)

# --- compute init ---
# --- apply temporal regularization along partitions ---
for (( B=0; B<${NBIN}; B++ )); do

	bart slice 10 $B traj_prep temp_traj_b
	bart slice 10 $B kdat_prep temp_kdat_b
...
for (( P=0; P<${NPAR}; P++ )); do
	...
	bart slice 13 $P traj_prep temp_traj_p
	bart slice 13 $P kdat_prep temp_kdat_p

	bart slice 13 $P R_INIT_M${MODEL_IND} temp_init_p

	bart moba -G -m${MODEL_IND} -rQ:1 -rS:0 -rW:3:$(bart bitmask 6):1 -u0.01 -i8 -C100 -R3 -d4 -g -k --kfilter-2 -o$OVERGRID -I temp_init_p -t temp_traj_p temp_kdat_p ${TE} R_M${MODEL_IND}_P${P}
done
